...
@onready var _world: TileMapLayer = get_tree().get_nodes_in_group("world")[0] as TileMapLayer
...
var i_can_step_forward: bool:
	get: return _world.get_cell_tile_data(coordinates_before_me) != null
...
var tile_color_before_me: StringName:
	get:
		var tile: TileData = _world.get_cell_tile_data(coordinates_before_me)
		if tile == null:
			return ""
...
var tile_color_under_me: StringName:
	get:
		var tile: TileData = _world.get_cell_tile_data(coordinates)
		if tile == null:
			return ""
		return tile.get_custom_data("tile_color")
